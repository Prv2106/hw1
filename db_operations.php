...
function showFavorites(){
    ...
    if(mysqli_num_rows($res) > 0){
        ...
        while($row = mysqli_fetch_assoc($res)){
            $shared = false;
            $shared_query = "SELECT * from chat where binary username = '".$username."'  AND  movie_id ='".$row['movie_id']."' ";
            $shared_res = mysqli_query($conn,$shared_query);
            if(mysqli_num_rows($shared_res) > 0){
                $shared=true;
            }
            else{
                $shared=false;
                ...
                $results[]= array("res" => $row,"shared"=> $shared);
            }
        $status = true;
    }
    else{
